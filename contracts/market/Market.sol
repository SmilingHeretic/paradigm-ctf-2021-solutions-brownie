...
contract CryptoCollectibles {
    ...
    function mint(address tokenOwner) external returns (bytes32) {
        ...
        bytes32 tokenId = keccak256(
            abi.encodePacked(address(this), tokenIdSalt++)
        );
    ...
    function transfer(bytes32 tokenId, address to) external {
        require(
            msg.sender == eternalStorage.getOwner(tokenId),
            "transfer/not-owner"
        );
    ...
    function transferFrom(
        ...
        require(
            from == eternalStorage.getOwner(tokenId),
            "transferFrom/not-owner"
        );
        require(
            msg.sender == eternalStorage.getApproval(tokenId),
            "transferFrom/not-approved"
        );
    ...
    function getTokenMetadata(bytes32 tokenId)
        external
        view
...
contract CryptoCollectiblesMarket {
    ...
    function sellCollectible(bytes32 tokenId) public payable {
        ...
        (, address tokenOwner, address approved, ) = cryptoCollectibles
